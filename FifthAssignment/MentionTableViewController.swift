...
class MentionTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .keyword(let keyword):
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "", for: indexPath) as? MentionKeywordTableViewCell else { return UITableViewCell() }
            cell.label = keyword
            return cell
        case .image(let (url, _)):
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "", for: indexPath) as? MentionImageTableViewCell else { return UITableViewCell() }
            cell.url = url
            return cell
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == SegueIds.showFullScreenImage {
            guard let vc = segue.destination as? FullScreenImageViewController else {
                assertionFailure()
                return
            }
            ...
            guard let vc = segue.destination as? TweetTableViewController else {
                assertionFailure()
                return
            }
...
extension MentionTableViewController {
