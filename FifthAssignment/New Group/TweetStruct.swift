...
public struct TweetStruct : CustomStringConvertible
{
    ...
    public let hashtags: [TweetMention]
    public let urls: [TweetMention]
    public let userMentions: [TweetMention]
    ...
    private static func mentions(from twitterData: NSArray?, in text: String, with prefix: String) -> [TweetMention] {
        var mentions = [TweetMention]()
        for mentionData in twitterData ?? [] {
            if let mention = TweetMention(from: mentionData as? NSDictionary, in: text as NSString, with: prefix) {
...
public struct TweetMention: CustomStringConvertible
{
